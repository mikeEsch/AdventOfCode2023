...
int main(void) {
    ...
    unsigned int i;
    printf("\n ---- \n");
    for( i=0; i<(numSeeds-1); i+=2) {
        unsigned long start = seeds[i];
        unsigned long stop = start + seeds[i+1];
        for (unsigned long seed = start; seed < stop; seed++) {
            //printf("seed: %lu\n", seed);
            ...
            if (loc > location) {
                loc = location;
                printf("new closest location: %lu\n", loc);
            }
